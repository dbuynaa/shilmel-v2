...
import { db } from "@/db";
import { images, productVariants, products } from "@/db/schema";
...
export async function addItem(input: z.infer<typeof itemSchema>): Promise<"success" | "error"> {
	try {
...
export async function checkItem(input: {
	...
	try {
		const existingItem = await db
			...
			.from(products)
			.where(
				input.id ? eq(products.name, input.name) && eq(products.id, input.id) : eq(products.name, input.name),
			)
			.limit(1);
...
export async function getImagesByVariantId(slug: string) {
	noStore();
	try {
		const variantImages = await db.select().from(images).where(eq(images.variantId, slug));
