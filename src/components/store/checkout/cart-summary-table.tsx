	...
	const [optimisticCart, dispatchOptimisticCartAction] = useOptimistic(
		...
		(
			...
			const updatedItems = prevCart.items.map((item) => {
				if (item.id === action.productId && item.variant?.sku === action.variant) {
				...
				{optimisticCart.items.map((item) => (
					<TableRow key={item.variant?.sku}>
						...
						<TableCell className="font-medium">
							{item.name}
							{item.variant?.sku && <span className="text-muted-foreground ml-1">({item.variant.sku})</span>}
						</TableCell>
						<TableCell>
							{formatMoney({
								amount: item.price,
						...
						<TableCell>
							<div className="flex items-center gap-2">
								<Button
									...
									size="icon"
									className="h-8 w-8"
									onClick={() => handleQuantityChange(item.id, item.variant!.sku, "DECREASE")}
								>
								...
								<Button
									...
									size="icon"
									className="h-8 w-8"
									onClick={() => handleQuantityChange(item.id, item.variant!.sku, "INCREASE")}
								>
