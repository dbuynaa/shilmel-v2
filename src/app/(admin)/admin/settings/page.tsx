	...
	return (
		<div className="grid max-w-8xl grid-flow-row grid-cols-1 gap-5 p-5 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
			{settingsOptions.map((option) => {
				...
				return (
					<div key={option.title} className="size-full min-w-[300px] space-y-2 rounded-md border bg-tertiary">
						<div className="flex items-center gap-2 border-b px-5 py-3">
							<Icon className="size-5 text-muted-foreground" />
							<h2 className="whitespace-nowrap text-lg font-semibold tracking-wide">{option.title}</h2>
						</div>
