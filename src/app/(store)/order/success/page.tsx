...
export default async function OrderDetailsPage(props: {
	...
	const isDigital = (items: typeof order.items) => {
		return items.some((item) => Boolean(item.product.description?.includes("digital")));
	};
	...
	return (
		<article className="max-w-3xl pb-32">
			...
			<dl className="mt-12 space-y-2 text-sm">
				<dt className="text-foreground font-semibold">{t("orderNumberTitle")}</dt>
							...
							<footer className="row-start-3 mt-2 self-end">
								<dl className="grid grid-cols-[max-content_auto] gap-2 sm:grid-cols-3">
									<div className="max-sm:col-span-2 max-sm:grid max-sm:grid-cols-subgrid">
										<dt className="text-foreground text-sm font-semibold">{t("price")}</dt>
									...
									<div className="max-sm:col-span-2 max-sm:grid max-sm:grid-cols-subgrid">
										<dt className="text-foreground text-sm font-semibold">{t("quantity")}</dt>
										<dd className="text-accent-foreground text-sm">{item.quantity}</dd>
									</div>

									<div className="max-sm:col-span-2 max-sm:grid max-sm:grid-cols-subgrid">
										<dt className="text-foreground text-sm font-semibold">{t("total")}</dt>
										<dd className="text-accent-foreground text-sm">
											{formatMoney({
			...
			<div className="pl-20 sm:pl-40">
				...
				{isDigital(order.items) && (
					<div className="mb-8">
						<h3 className="leading-none font-semibold text-neutral-700">Digital Asset</h3>
						<ul className="mt-3">
							{order.items
								.filter((item) => item.product.description?.includes("digital"))
								.map((item) => {
									const downloadUrl = item.product.description?.match(/https?:\/\/[^\s]+/)?.[0];
				...
				<div className="grid gap-8 sm:grid-cols-2">
					{!isDigital(order.items) && order.shippingAddress && (
						<div>
							<h3 className="leading-none font-semibold text-neutral-700">{t("shippingAddress")}</h3>
					...
					<div className="col-span-2 grid grid-cols-2 gap-8 border-t pt-8">
						<h3 className="leading-none font-semibold text-neutral-700">{t("total")}</h3>
						<p>
							{formatMoney({
