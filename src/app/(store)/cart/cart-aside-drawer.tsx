...
export const CartAsideDrawer = ({ children }: { children: ReactNode }) => {
	...
	return (
		<Drawer open={open} shouldScaleBackground={true} direction={isDesktop ? "right" : "bottom"}>
			...
			<DrawerContent
				className="sm:bg-background sm:fixed sm:top-0 sm:right-0 sm:bottom-0 sm:left-auto sm:mt-0 sm:flex sm:h-full sm:w-1/2 sm:flex-col sm:overflow-hidden sm:rounded-none sm:shadow-xl lg:w-1/3"
